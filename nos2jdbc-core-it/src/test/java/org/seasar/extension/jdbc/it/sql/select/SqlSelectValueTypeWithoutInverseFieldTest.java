...
class SqlSelectValueTypeTestWithoutInverseField {
    ...
    void bean_temporalType() throws ParseException  {
        ...
        Tense tense = jdbcManager.selectBySql(Tense.class, sql).getSingleResultWithoutInverseField();
    ...
    void bean_temporalType_Calendar() throws ParseException  {
        ...
        Tense tense = jdbcManager.selectBySql(Tense.class, "SELECT * FROM TENSE WHERE CAL_DATE = ?", date(calendar)).getSingleResultWithoutInverseField();
        assertNotNull(tense);
        calendar.setTime(new SimpleDateFormat("HH:mm:ss").parse("12:11:10"));
        tense = jdbcManager.selectBySql(Tense.class, "SELECT * FROM TENSE WHERE CAL_TIME = ?", time(calendar)).getSingleResultWithoutInverseField();
        assertNotNull(tense);
        calendar.setTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse("2005-02-14 12:11:10"));
        tense = jdbcManager.selectBySql(Tense.class, "SELECT * FROM TENSE WHERE CAL_TIMESTAMP = ?", timestamp(calendar)).getSingleResultWithoutInverseField();
        assertNotNull(tense);
    }
    ...
    void bean_temporalType_Date() throws ParseException  {
        ...
        Tense tense = jdbcManager.selectBySql(Tense.class, "SELECT * FROM TENSE WHERE DATE_DATE = ?", date(date)).getSingleResultWithoutInverseField();
        assertNotNull(tense);
        date = new SimpleDateFormat("HH:mm:ss").parse("12:11:10");
        tense = jdbcManager.selectBySql(Tense.class, "SELECT * FROM TENSE WHERE DATE_TIME = ?", time(date)).getSingleResultWithoutInverseField();
        assertNotNull(tense);
        date = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse("2005-02-14 12:11:10");
        tense = jdbcManager.selectBySql(Tense.class, "SELECT * FROM TENSE WHERE DATE_TIMESTAMP = ?", timestamp(date)).getSingleResultWithoutInverseField();
        assertNotNull(tense);
    }
    ...
    void bean_temporalType_Jdbc42() throws ParseException  {
        Date date = new SimpleDateFormat("yyyy-MM-dd").parse("2005-02-14");
        Tense tense = jdbcManager.selectBySql(Tense.class, "SELECT * FROM TENSE WHERE LOCAL_DATE = ?", date(date)).getSingleResultWithoutInverseField();
        assertNotNull(tense);
        date = new SimpleDateFormat("HH:mm:ss").parse("12:11:10");
        tense = jdbcManager.selectBySql(Tense.class, "SELECT * FROM TENSE WHERE LOCAL_TIME = ?", time(date)).getSingleResultWithoutInverseField();
        assertNotNull(tense);
        date = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse("2005-02-14 12:11:10");
        tense = jdbcManager.selectBySql(Tense.class, "SELECT * FROM TENSE WHERE LOCAL_DATE_TIME = ?", timestamp(date)).getSingleResultWithoutInverseField();
        assertNotNull(tense);
    }
    ...
    void map_temporalType()  {
        String sql = "SELECT * FROM TENSE WHERE ID = 1";
        Map<?, ?> tense = jdbcManager.selectBySql(Map.class, sql).getSingleResultWithoutInverseField();
        assertNotNull(tense);
    ...
    void object_temporalType() throws ParseException   {
        ...
        Calendar calTimestamp = jdbcManager.selectBySql(Calendar.class, sql).temporal(TemporalType.TIMESTAMP).getSingleResultWithoutInverseField();
